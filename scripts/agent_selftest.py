...
try:
    from env_resolver import get_plantilla_dir, get_projects_dirs, get_repo_root
...
def test_workflows(project_dir: Path) -> dict:
    ...
    results["session_protocol"] = (
        (rules_dir / "session-protocol.md").is_file() if rules_dir.is_dir() else False
    )
...
def test_governance(project_dir: Path) -> dict:
    ...
    # Output governance
    results["output_governance"] = (
        project_dir / "docs" / "standards" / "output_governance.md"
    ).is_file()
...
def compute_readiness_score(
    dispatch: dict, workflows: dict, memory: dict, governance: dict
) -> int:
...
def run_selftest(projects: list[tuple[str, Path]]) -> None:
    ...
    for name, path in projects:
        ...
        print(
            f"    Dispatch:    {'✅' if dispatch['dispatch_script'] else '❌'} script"
            f" | {'✅' if dispatch['manifest_valid'] else '❌'} manifest"
            f" ({dispatch['agent_count']} agents)"
        )
        print(
            f"    Workflows:   {workflows.get('workflow_count', 0)} workflows"
            f" | {'✅' if workflows['session_protocol'] else '❌'} session-protocol"
        )
        print(
            f"    Memory:      {'✅' if memory['brain_dir'] else '❌'} brain"
            f" | {memory.get('skill_count', 0)} skills"
        )
        print(
            f"    Governance:  {'✅' if governance['gemini_has_rules'] else '❌'} rules"

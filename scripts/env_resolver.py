...
def get_projects_dirs() -> list[Path]:
    """Return the list of project directories (domains) for the current environment."""
    _, env_cfg = detect_environment()
    base = get_repo_root()

    # In the new naming convention, domains start with digits (00_, 01_, etc.)
    domain_dirs = [d for d in base.iterdir() if d.is_dir() and d.name[:2].isdigit()]
    if domain_dirs:
...
def get_plantilla_dir() -> Path:
    ...
    _, env_cfg = detect_environment()
    base = get_repo_root()
    # Path is hardcoded in 00_CORE for the desktop/new setup
    plantilla_path = base / "00_CORE" / env_cfg["plantilla_dir"]
    ...
    if not legacy_path.exists():
        print(
            f"[CRITICAL: PATH UNREACHABLE] AG_Plantilla not found at {plantilla_path} or {legacy_path}.",
            file=sys.stderr,
        )
        sys.exit(1)
    return legacy_path
...
def list_ag_projects() -> list[Path]:
    ...
    projects = []
    for domain_dir in get_projects_dirs():

...
KNOWN_CREDENTIAL_HASHES = {
    "0537dfd229ccd644e29c82f0c27a1b3b075a1589fa75a1761803738e4a905be2": {
        "label": "default secret key",
        "severity": "high",
    },
    "cf79dedfbae3ccda454685ff8619bc306c59bd02aaea395cb28205bd0714edec": {
        "label": "SIDRA DB password",
        "severity": "critical",
    },
    "11881f383d4473e94f958fff628d09f66dc0fa80be9ae7ba1cd6010534da8ec5": {
        "label": "IRIS DB password",
        "severity": "critical",
    },
...
def is_scannable_file(filepath: Path) -> bool:
    """Return True when a file extension/name is in scan scope."""
    # Path(".env").suffix == "", so dotfiles need explicit name matching.
    if filepath.suffix in SCAN_EXTENSIONS:
        return True
    return filepath.name == ".env" or filepath.name.startswith(".env.")
...
def scan_security(project_dir: Path) -> list[dict]:
    ...
    for filepath in project_dir.rglob("*"):
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            # Check known credential hashes by tokenizing the line
            words = re.split(r"[^a-zA-Z0-9_\-]", line)
            for w in words:
                if not w:
                    continue
                w_hash = hashlib.sha256(w.encode("utf-8")).hexdigest()
                if w_hash in KNOWN_CREDENTIAL_HASHES:
                    cred = KNOWN_CREDENTIAL_HASHES[w_hash]
                    ...
                    break  # Avoid duplicate findings for the same line
...
def print_results(results: list[dict]) -> None:
    ...
    print(
        ...
        f"{len(KNOWN_CREDENTIAL_HASHES)} known creds | {len(GENERIC_PATTERNS)} generic patterns"
    )
    print("=" * 70)

...
def parse_devlog_all() -> list[dict]:
    ...
    sessions_raw = [
        s.strip() for s in sessions_raw if s.strip() and s.startswith("## 20")
    ]
...
def sync_to_vault() -> None:
    ...
    if KnowledgeVault is None:
        print("  Vault: core.vault not available, skipping SQLite sync")
        return
    ...
    print(
        f"  Vault: {total_stored} new entries stored ({len(sessions)} sessions processed)"
    )


def parse_todo() -> dict:
    """Extract current TASKS.md state."""
...
def generate_devlog_from_git(topic: str = "Session") -> None:
    ...
    content = (
        DEVLOG_PATH.read_text(encoding="utf-8")
...
def generate_team_context(description: str) -> None:
    ...
    if db_path.exists() and KnowledgeVault is not None:
        try:
            vault = KnowledgeVault(str(PROJECT_ROOT))
...
def compact_devlog(keep_recent: int = 10) -> None:
    ...
    print(
        f"[i] Compacting: keeping {len(recent)} recent, archiving {len(older)} older sessions"
    )
...
def main():
    ...
    if "--query" in sys.argv:
        ...
        if KnowledgeVault is None:
            print("Knowledge Vault is not available. Please install core.vault.")
            return
    ...
    print(
        f"📋 TASKS: {len(todo['blockers'])} blockers, {len(todo['in_progress'])} in-progress, {len(todo['backlog'])} backlog"
    )
